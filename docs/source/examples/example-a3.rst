...
Unlike :doc:`Example A-4 <example-a4>`, here we manipulate the elements one by one.
...
The *Non-Empty* elements (which happens to be MAT-1 and MAT-2), are requested to be exported.

The code can be found in the *examples* folder of the main repository. It is also included below:

.. literalinclude:: /../../examples/ex_a3_manual_manipulation_single.py

The final model looks like :numref:`ex_a3_manual_manipulation_single`.
Note that elements in different sets are shown in different colors.

.. figure:: /images/ex_a3_manual_manipulation_single.png
   :name: ex_a3_manual_manipulation_single
   :align: center
   :alt: Final model of Example A-3.
         Element sets corresponding to MAT-1 and MAT-2 are shown in green and red, respectively.

   Final model of Example A-3.
   Element sets corresponding to MAT-1 and MAT-2 are shown in green and red, respectively.